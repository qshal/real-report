...
import { storeNewsVerification, generateArticleHash } from "@/lib/blockchain";
import { storeVerificationViaApi, checkDuplicateVerification } from "@/lib/blockchainApi";
...
export const analyzeNewsHybrid = async (payload: AnalyzeNewsPayload): Promise<HybridAnalysisResult> => {
  ...
  // Check for duplicate verification first (privacy-preserving)
  const duplicateCheck = await withTimeout(
    checkDuplicateVerification(contentToAnalyze),
    5000,
    { isDuplicate: false }
  );

  if (duplicateCheck.isDuplicate && duplicateCheck.previousVerification) {
    const prev = duplicateCheck.previousVerification;
    return {
      label: prev.is_real ? "real" : "fake",
      confidence: prev.confidence,
      explanation: `Previously verified: ${prev.is_real ? 'REAL' : 'FAKE'}. Original verification on ${prev.verification_date}.`,
      modelName: "blockchain-duplicate",
      blockchain: {
        contentHash: prev.article_hash,
        resultHash: prev.article_hash, // Using article hash as result hash for simplicity
        stored: true,
        verified: true,
      },
      metadata: {
        ...
        reasoning: "Article was previously verified on blockchain - returning cached result",
      },
    };
  }
  ...
  // TruthChain blockchain integration - generate hash and attempt storage
  const timestamp = Math.floor(Date.now() / 1000);
  const contentForHash = isUrl ? `${payload.url}|${urlContent?.title || ''}|${urlContent?.content || ''}` : contentToAnalyze;
  const articleHash = generateArticleHash(contentForHash);
  
  let blockchainResult = {
    contentHash: articleHash,
    resultHash: articleHash, // Using article hash as result hash
    stored: false,
    verified: false,
  };

  // Try to store on blockchain via backend service (non-blocking)
  try {
    const storeResult = await withTimeout(
      storeVerificationViaApi(
        contentForHash, 
        label, 
        adjustedConfidence,
        finalTrustScore / 100,
        isUrl ? payload.url : undefined,
        "multi_model_ensemble"
      ),
      10000,
      { success: false, error: 'Blockchain storage timeout' }
    );
    ...
    if (storeResult.success) {
      ...
      blockchainResult = {
        ...
        stored: true,
        verified: true,
        error: 'Duplicate verification found',
      };
    } else {
      blockchainResult = {
        ...blockchainResult,
  ...
  return {
    ...
    metadata: {
      ...
      // TruthChain verification info
      truthchainVerification: {
        articleHash: blockchainResult.contentHash,
        stored: blockchainResult.stored,
        txHash: blockchainResult.txHash,
        timestamp,
        network: "TruthChain",
        privacyPreserving: true,
        immutableRecord: blockchainResult.stored,
      },
    },
  };
};
